...
// @version      1.0.0
// @description  Темная тема, Завеса демонстрации, Дискотека, невидимый ник, прозрачная аватарка, WS спам, Мониторинг, Спам рукой, Инспектор митинга
...
(() => {
  ...
  function doRename(newName, oldName, req, store) {
  ...
  async function runStora(tempNick, req, store, iwin, autoSetup = false, offset = 0) {
    ...
    const doDraw = () => {
      if (!drawFillRect(iwin, offset)) {
        showStatus(T('❌ Открой аннотацию в Zoom!', '❌ Open annotation in Zoom!'), '#ef4444');
        return;
      }
