...
### 2.6. Рулетка ников (Хамелеон / Mimic Roulette)
* **Принцип работы:** Модуль позволяет непрерывно маскироваться под участников конференции в реальном времени.
* **Выборка участников (`getOtherAttendees`):** Извлекает массив присутствующих из `store.getState().attendeesList.attendeesList` и строго фильтрует текущего пользователя по постоянному `userId` (`getCurrentUser(store)?.userId`), исключая дублирование своего имени.
* **Случайный выбор без повторов (`pickRandomAttendeeNick`):** Если в конференции $\ge 2$ других участников, скрипт выбирает случайного человека, гарантируя отсутствие двух одинаковых ников подряд.
* **Пресеты скорости и предел сокетов Zoom:**
  * `⚡ 200мс (Турбо)`: 5 запросов `WS_CONF_RENAME_REQ` в секунду. Это физический максимум, который веб-клиент Zoom способен обработать в WebSocket шлюзе до накопления очереди. Меньшие задержки (<150мс) приводят к отбрасыванию пакетов сервером.
  * `🚀 400мс (Быстро)`: 2.5 запроса в секунду.
  * `⏱ 750мс (Оптимально)`: базовый сбалансированный режим.
  * `🐢 1500мс (Спокойно)`: плавный фоновый режим.
  * Переключение скорости происходит мгновенно на лету без остановки рулетки через функцию `setMimicSpeed(ms)`.
* **Управление и горячая выгрузка:** В интерфейсе представлен удобный iOS-переключатель (`.zt-switch`). При отключении или перезагрузке виджета (`window.__zt_cleanup`) интервальный таймер `_mimicTimer` гарантированно сбрасывается.

### 2.7. Горячая перезагрузка (Hot-Reload)
